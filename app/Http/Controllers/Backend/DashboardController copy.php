...
class DashboardController extends Controller
{
    ...
    public function moduleList()
    {
        ...
        if ($moduleId === null) {
            // Handle the case where $moduleId is null, for example, by redirecting or displaying an error message
            return redirect()->back()->with('error', 'Module ID not found.');
        }

        $permissionData = RolePermission::where('role_id', Auth::user()->role_id)
            ->where('module_id', $moduleId->id)
            ->where('status', 1)
            ->first();
